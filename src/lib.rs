//! - [`create_numpy_array`]: Create a numpy_array file, using memory mapping  
//! - [`load_numpy_array`]: Load a numpy_array from a file, using memory mapping 
//! - [`to_numpy_array`]: Take ownership of a rust vector and convert it to a numpy array, properly handling its memory allocation
//! 
//! The flat vectors have to follow the same [convention of numpy](https://numpy.org/doc/1.21/reference/internals.html).
//! 
use numpy::*;
...
/// Create a new numpy array. If a path is provided, this will create a file
/// memory mapping it to memory.
...
/// Memory map a numpy array from disk
pub fn load_numpy_array(
    py: Python,
    path: &str,
) -> (NPY_TYPES, bool, Py<PyAny>) {
    // mmap the file
    let mut mmap = MemoryMapped::new_mut(Some(path), None, None).expect("Could not mmap the file");
